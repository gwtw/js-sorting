...
function sort(array, compare, shift) {
  for (var i = 1; i < array.length; i++) {
    var item = array[i];
    var indexHole = i;
    while (indexHole > 0 &&
        compare(array[indexHole - 1], item) > 0) {
...
module.exports = function (array, customCompare, customShift) {
  var compare = customCompare || defaultCompare;
  return sort(array, compare, customShift);
};
